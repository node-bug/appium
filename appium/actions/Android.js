...
class Android extends ActionsBase {
  ...
  async source() {
    return this.driver.executeScript('mobile: source', [{ format: 'xml' }])
  }
  ...
  async swipe(elementId, direction, percent = 0.8) {
    if (!['up', 'down', 'left', 'right'].includes(direction)) {
      throw new Error(
        "Invalid swipe direction. Use 'up', 'down', 'left', or 'right'.",
      )
    }
  ...
  async pressKeyCode(keyCode) {
    this.driver.pressKeyCode(keyCode)
  }

  async search() {
    this.driver.executeScript('mobile: performEditorAction', [
      { action: 'search' },
    ])
  }
}
