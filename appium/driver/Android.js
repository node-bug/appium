...
class Android extends Strategy {
  ...
  //   this.message = `${this.message} and ${this.messenger({
  //     stack: e,
  //     action: 'drop',
  //   })}`
  ...
  async text() {
    ...
    try {
      const locator = await this.finder()
      const v = await this.device.actions.getAttribute(locator.ELEMENT, 'text')
      log.info(`Text is ${v}`)
      this.stack = []
      return v
    } catch (err) {
  ...
  async swipeUp() {
    ...
    const element = await this.device.driver.findElement(
      'xpath',
      '(//android.view.View)[3]',
    )
    await this.device.actions.swipe(element.elementId, 'up')
    await this.waitToRecover(this.RECOVERY_TIME)
    return true
  }
  ...
  async swipeRight() {
    this.message = this.messenger({ stack: this.stack, action: 'scrollRight' })
    const element = await this.device.driver.findElement(
      'xpath',
      '(//android.view.View)[3]',
    )
    await this.device.actions.swipe(element.elementId, 'right')
    await this.waitToRecover(this.RECOVERY_TIME)
    return true
  }

  async swipeLeft() {
    this.message = this.messenger({ stack: this.stack, action: 'scrollLeft' })
    const element = await this.device.driver.findElement(
      'xpath',
      '(//android.view.View)[3]',
    )
