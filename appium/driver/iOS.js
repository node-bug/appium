...
class iOS extends Strategy {
    ...
    async write(value) {
        this.message = this.messenger({ stack: this.stack, action: 'write', data: value })
        this.stack[0].visible = true
        try {
            const locator = await this.finder(null, 'write')
    ...
    async isNotDisplayed(t = null) {
        ...
        this.message = this.messenger({ stack: this.stack, action: 'isNotDisplayed' })
        let timeout
        if (t === null) {
    ...
    async value() {
        ...
        try {
            const locator = await this.finder()
            const value = await this.device.actions.getAttribute(locator.ELEMENT, 'value')
