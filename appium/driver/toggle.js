...
class Toggle {
  ...
  async action(state) {
    this.that.message = this.that.messenger({stack: this.that.stack, action: 'toggle', data: state })

    try {
      const locator = await this.that.finder(null, 'toggle')
      const currentState = await this.that.device.actions.getAttribute(locator.ELEMENT, 'value')
      if (
        (state === 'ON' && currentState === '0') ||
        (state === 'OFF' && currentState === '1')
      ) {
        await this.that.device.actions.tap(locator.ELEMENT)
        ...
        try {
          ...
          newState = await this.that.device.actions.getAttribute(newLocator.ELEMENT, 'value')
        } catch (err) {
          log.warn(`${this.that.message}\n${err.message}`)
          log.info(
            `Retrying to get value of toggle using ElementId before toggle was tapped on.`,
          )
          newState = await this.that.device.actions.getAttribute(locator.ELEMENT, 'value')
        }
  ...
  async validate(expectedState) {
    let currentState
    this.that.message = this.that.messenger({ stack: this.that.stack, action: 'value' })
    try {
      const locator = await this.that.finder(null, 'toggle')
      currentState = await this.that.device.actions.getAttribute(locator.ELEMENT, 'value')
    } catch (err) {
      log.error(
