...
class DeviceBase {
  constructor(server, capability) {
    ...
    this.defaultTimeout = timeout || 10
    ...
    this.options = {
      ...
      connectionRetryTimeout: this.defaultTimeout * 10000,
    }
  }
  ...
  async start() {
    try {
      ...
      if (["Failed to create session.\nfetch failed"].includes(err.message)
        ...
        const message = 'Appium session could not be created. Either the Appium server is not started or device does not exist.'
        log.warn(message)
  ...
  async activate() {
    throw new Error(`Function ${this.name} "activate(packageName)" is not implemented`)
  }

  async terminate() {
    throw new Error(`Function ${this.name} "terminate(packageName)" is not implemented`)
  }

  async uninstall() {
    throw new Error(`Function ${this.name} "uninstall(packageName)" is not implemented`)
  }
  ...
  async startScreenRecording() {
    throw new Error(`Function ${this.name} "startScreenRecording" is not implemented`)
  }

  async stopScreenRecording() {
    throw new Error(`Function ${this.name} "stopScreenRecording" is not implemented`)
  }
