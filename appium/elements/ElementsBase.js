            ...
            matches = (
                await Promise.all(
                    elements.map(async (element) => {
                        ...
                        return this.nearestElement(locator, elementData.type)
                    }),
                )
    ...
    async nearestElement(locator, type = null) {
        ...
        await Promise.all(
            tagnames.map(async (tagname) => {
                const elementsFound = await this.driver.findElements(
                    'xpath',
                    `//${tagname}`,
                )
        ...
        elements = elements.filter((e) => !locator.visible || e.visible)
